...
static struct BitmapGlyph* find_glyph(struct BitmapFont* font, uint32_t char_code) {
    int i;
...
static struct BitmapGlyph* add_glyph(struct BitmapFont* font, uint32_t char_code) {
    struct BitmapGlyph* glyph;
    FT_Face face;
    int index, x, y;
...
static const struct BitmapGlyph* load_glyph(struct BitmapFont *font, uint32_t char_code) {
...
static uint32_t utf8_to_utf32(const char* s, int* len) {
    uint32_t out = 0;
    uint8_t c;
    int nb;
