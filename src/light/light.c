...
void light_load_direct_uniforms(GLuint shader, const struct Lights* lights) {
    ...
    for (i = 0; i < lights->numDirectionalLights; i++) {
        ...
        glUniform3fv(glGetUniformLocation(shader, locName), 1, l->direction);
        sprintf(locName, "directionalLights[%d].color", i);
        glUniform3fv(glGetUniformLocation(shader, locName), 1, l->color);
    }

    for (i = 0; i < lights->numPointLights; i++) {
        const struct PointLight* l = lights->point + i;
        sprintf(locName, "pointLights[%d].position", i);
        glUniform3fv(glGetUniformLocation(shader, locName), 1, l->position);
    ...
    glUniform3fv(glGetUniformLocation(shader, "ambientLight.color"), 1, lights->ambientLight.color);
}
